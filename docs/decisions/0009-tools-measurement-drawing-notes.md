...
The top and bottom toolbar surfaces are translucent. The tabletop canvas fills the complete Scaffold instead of being inset to the toolbar-safe content area, so the map and grid continue to render behind both toolbar regions. Floating menus and the total-measurement readout still respect the toolbar insets.
...
- A marker itself is also a direct movement handle. Dragging an existing marker changes its position and immediately updates the connected segments; Snap enabled follows the normal grid anchors and Snap disabled remains free.
- Tapping an existing marker without dragging selects it and opens a contextual delete action.
- Deleting a selected marker removes that marker and every marker created after it. Earlier markers remain in order.
- Each segment displays its own formatted distance near the segment on the tabletop.
- The bottom measurement readout displays only the aggregate total distance/grid-step count for the entire path.
- Clear measurement removes the complete path.

The selected marker index and any active pointer gesture are transient UI state and are not restored after app restart.

## Drawing

Completed drawing strokes store their own ARGB color in addition to world-space points and brush width.
...
- The existing Clear menu includes **Clear notes** when at least one notecard exists.
...
Version 2 persists measurement paths, per-stroke colors, current brush color, and note IDs/text/positions along with the existing tabletop state. Measurement marker movement does not require another schema change because marker positions are already represented by the persisted path point list.

## Deferred work

Measurement path reordering, configurable brush width, configurable eraser size, note colors/styles, note locking, rich text, note layering, and note attachments are deferred.
